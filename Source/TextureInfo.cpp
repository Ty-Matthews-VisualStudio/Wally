...
void CTextureInfo::Init (LPQ2_MIP_S WalHeader)
{
	...
	_itoa (WalHeader->width, buffer, 10);	
	m_TextureWidth = buffer;
	
	_itoa (WalHeader->height, buffer, 10);	 
...
void CTextureInfo::OnOK() 
{
	...
	UpdateData(true);
	strcpy (DocWalHeader->animname,(LPCSTR)m_AnimationName);
	strcpy (DocWalHeader->name,(LPCSTR)m_TextureName);
